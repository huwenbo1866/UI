<script lang="ts">
	...
	async function handleRewardAnswer(
		...
		if (isCorrect) {
			...
			await markWrongNotebookCorrect(pack.id, choice.question.id, pack.source).catch(
				() => undefined
			);
			...
			await recordWrongNotebookEntry(pack.id, choice.question, selected, pack.source).catch(
				() => undefined
			);
			closeRewardPanel(state);
			rewardPanelOpenedFromPending = false;
