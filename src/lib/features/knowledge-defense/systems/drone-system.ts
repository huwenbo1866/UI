...
const DRONE_ENGAGE_BUFFER = KD_DRONE_TUNING.engageBuffer;
const DRONE_PREFERRED_RANGE = DRONE_ENGAGE_RANGE * KD_DRONE_TUNING.preferredRangeRatio;
const DRONE_MIN_COMBAT_RANGE = DRONE_ENGAGE_RANGE * KD_DRONE_TUNING.minimumCombatRangeRatio;
const DRONE_TARGET_STICKINESS_BONUS = KD_DRONE_TUNING.targetStickinessBonus;
...
function moveTowardsPoint(
	...
	const next = moveToward(
		...
		targetX,
		targetY,
		DRONE_SPEED * speedScale * (drone.moveSpeedMultiplier ?? 1),
		dtSeconds,
		slowRadius
	);
	drone.x = next.x;
...
function executeAttack(state: GameState, drone: DroneState, target: MonsterState) {
	...
	state.lasers.push({
		...
		ttlMs: KD_DRONE_TUNING.laserTtlMs
	});
	if (state.lasers.length > KD_WEAPON_TUNING.laser.maxActive) {
		state.lasers.splice(0, state.lasers.length - KD_WEAPON_TUNING.laser.maxActive);
	}

	drone.cooldownMs =
		DRONE_COOLDOWN_MS /
		Math.min(KD_DRONE_TUNING.attackSpeedUpgradeCap, drone.attackSpeedMultiplier ?? 1);
