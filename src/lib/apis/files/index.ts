...
export const updateFileMetaById = async (token: string, id: string, meta: object) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/files/${id}/meta/update`, {
		method: 'POST',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
...
export const getFileChapterHomeworks = async (token: string, id: string) => {
	let error = null;
...
export const updateFileChapterHomework = async (
	token: string,
	id: string,
	homeworkId: string,
	payload: { questions?: any[]; answer_markdown?: string }
) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/files/${id}/chapter-homeworks/${homeworkId}`, {
		method: 'POST',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
