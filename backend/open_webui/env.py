...
PERSONALIZATION_BRAIN_CONFIG_PATH = Path(
    os.environ.get(
        "PERSONALIZATION_BRAIN_CONFIG_PATH",
        str(OPEN_WEBUI_DIR / "brain" / "config" / "personalization_brain.json"),
    )
).expanduser()

if not PERSONALIZATION_BRAIN_CONFIG_PATH.is_absolute():
    PERSONALIZATION_BRAIN_CONFIG_PATH = (
        BASE_DIR / PERSONALIZATION_BRAIN_CONFIG_PATH
    ).resolve()
else:
    PERSONALIZATION_BRAIN_CONFIG_PATH = PERSONALIZATION_BRAIN_CONFIG_PATH.resolve()
