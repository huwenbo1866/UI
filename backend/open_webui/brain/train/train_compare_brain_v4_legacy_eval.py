...
目标:
1. 保持旧评估方式不变（train_test_split + KFold/StratifiedKFold）
2. 对比 baseline(v2结构) 与 optimized(v3候选思路)
3. 产出独立 artifact / report，避免改动现有 v2/v3 流程
"""
...
def load_dataset(csv_path: str | Path) -> pd.DataFrame:
    ...
    df = df.dropna(subset=[REG_TARGET_COL, CLS_TARGET_COL]).copy()
...
def evaluate_regressors_cv(X: pd.DataFrame, y: pd.Series, models: dict[str, object]) -> list[RegressorResult]:
...
def generate_oof_and_fold_models(model_template: object, X: pd.DataFrame, y: pd.Series):
    cv = KFold(n_splits=REG_CV_SPLITS, shuffle=True, random_state=RANDOM_STATE)
...
def evaluate_classifiers_cv(X_stacked: pd.DataFrame, y_cls: pd.Series, models: dict[str, object]) -> list[ClassifierResult]:
...
def _build_oof_proba(model_template: object, X: pd.DataFrame, y: pd.Series) -> np.ndarray:
    cv = StratifiedKFold(n_splits=CLS_CV_SPLITS, shuffle=True, random_state=RANDOM_STATE)
...
def apply_scene_thresholds(
    ...
    if mode != "scene_bucketed":
        t = float(threshold_info.get("selected_threshold", 0.5))
...
def find_scene_adaptive_thresholds_with_oof(
    ...
    min_bucket_pos: int = 6,
    min_bucket_neg: int = 6,
) -> dict[str, object]:
    oof_proba = _build_oof_proba(model_template, X, y)
    thresholds = np.arange(0.30, 0.72, 0.02)

    y_true = y.to_numpy()
    global_threshold, global_f1, global_bal_acc = _select_best_threshold_by_f1(
        y_true,
        oof_proba,
        thresholds,
    )
    ...
    for bucket_name in bucket_names:
        mask = np.array([bucketize_score(float(s), score_bins) == bucket_name for s in score_values], dtype=bool)
        ...
        can_tune = sample_cnt >= min_bucket_samples and pos_cnt >= min_bucket_pos and neg_cnt >= min_bucket_neg

        if can_tune:
            t_bucket, f1_bucket, bal_acc_bucket = _select_best_threshold_by_f1(y_bucket, p_bucket, thresholds)
    ...
    threshold_info: dict[str, object] = {
        ...
        "global_oof_best_f1": float(global_f1),
        "global_oof_best_balanced_accuracy": float(global_bal_acc),
        "bucket_thresholds": bucket_thresholds,
        "bucket_stats": bucket_stats,
        "score_bins": score_bins,
    }

    bucket_pred = apply_scene_thresholds(oof_proba, score_values, threshold_info, score_bins)
...
def route_strategy(path_class: int, path_confidence: float, score: float, config: dict) -> str:
    if path_confidence < config["classifier_confidence_floor"]:
...
def train_and_eval_pipeline(
    ...
    if tune_threshold:
        if threshold_strategy == "scene_bucketed":
            runtime_score_bins = compute_adaptive_score_bins(train_reg_oof_score, ROUTING_CONFIG["score_bins"])
            threshold_info = find_scene_adaptive_thresholds_with_oof(
                best_cls_model,
                X_train_stacked,
                y_cls_train,
                train_reg_oof_score,
            ...
            thresholds = np.arange(0.30, 0.72, 0.02)
            oof_proba = _build_oof_proba(best_cls_model, X_train_stacked, y_cls_train)
            t_best, f1_best, bal_best = _select_best_threshold_by_f1(y_cls_train.to_numpy(), oof_proba, thresholds)
            threshold_info = {
                "mode": "global",
                "objective": "maximize_f1",
                "selected_threshold": float(t_best),
                "oof_best_f1": float(f1_best),
                "oof_best_balanced_accuracy": float(bal_best),
            }
            cls_test_pred = (cls_test_proba >= t_best).astype(int)
    else:
    ...
    return {
        ...
        "routing_config_runtime": runtime_routing_config,
...
def main() -> None:
    ...
    if not augmented_path.exists():
        raise FileNotFoundError(f"缺少扩增训练集: {augmented_path}，请先生成")

    original_df = load_dataset(original_path)
    augmented_df = load_dataset(augmented_path)

    # 旧口径: 仅做分层 holdout，不做 group-aware 切分
    indices = np.arange(len(original_df))
    idx_train, idx_test = train_test_split(
        indices,
        test_size=TEST_SIZE,
        random_state=RANDOM_STATE,
        stratify=original_df[CLS_TARGET_COL],
    )
    ...
    # 仅避免行级泄漏: 扩增训练集中去掉原始测试行 id
    if "id" in original_df.columns and "id" in augmented_df.columns:
        test_ids = set(origin_test_df["id"].astype(str).unique())
        optimized_train_df = augmented_df[~augmented_df["id"].astype(str).isin(test_ids)].reset_index(drop=True)
    else:
        optimized_train_df = augmented_df.copy()

    baseline_result = train_and_eval_pipeline(
        train_df=origin_train_df,
        test_df=origin_test_df,
        reg_candidates={"baseline_extra_trees": build_baseline_regressor()},
        cls_candidates={"baseline_extra_trees": build_baseline_classifier()},
        mode_name="baseline_v2_architecture_legacy_eval",
        tune_threshold=False,
    )

    optimized_result = train_and_eval_pipeline(
        train_df=optimized_train_df,
        test_df=origin_test_df,
        reg_candidates=build_regressor_candidates_optimized(),
        cls_candidates=build_classifier_candidates_optimized(),
        mode_name="optimized_v4_legacy_eval",
        tune_threshold=True,
    ...
    comparison = {
        "evaluation": {
            "strategy": "Legacy eval (Stratified holdout + KFold/StratifiedKFold)",
            "test_size": TEST_SIZE,
            "random_state": RANDOM_STATE,
            "primary_objective": "f1",
            "lightgbm_enabled": HAS_LIGHTGBM,
            "note": "保持旧评估口径，不使用 group-aware 切分",
        },
        "data": {
            ...
            "optimized_train_rows": int(len(optimized_train_df)),
        },
        "baseline": {
        ...
        "delta_optimized_minus_baseline": {
            "reg_test_r2": float(optimized_reg["test_r2"] - baseline_reg["test_r2"]),
            "reg_test_rmse": float(optimized_reg["test_rmse"] - baseline_reg["test_rmse"]),
            "cls_test_f1": float(optimized_cls["test_f1"] - baseline_cls["test_f1"]),
            "cls_test_accuracy": float(optimized_cls["test_accuracy"] - baseline_cls["test_accuracy"]),
            "cls_test_roc_auc": float(optimized_cls["test_roc_auc"] - baseline_cls["test_roc_auc"]),
        },
    }

    artifact = {
        "artifact_type": "xiaoling_personalization_brain",
        "version": "v4_legacy_eval_optimized",
        ...
        "routing_config": optimized_result["routing_config_runtime"],
    ...
    artifact_path = model_dir / "xiaoling_personalization_brain_v4_legacy_eval_optimized.joblib"
    report_path = model_dir / "xiaoling_personalization_brain_v4_legacy_eval_report.json"
    summary_path = model_dir / "xiaoling_personalization_brain_v4_legacy_eval_summary.md"
